...
Scope {
    ...
    function loadChatHistory() {
        var path = root.homeDir + "/.cache/quickshell/chat_sidebar/chats.json"
        var fileView = Qt.createQmlObject('import Quickshell.Io; FileView { path: "file://' + path + '"; watchChanges: true }', root)
        if (fileView && fileView.text && fileView.text.trim().length > 0) {
            try {
                return JSON.parse(fileView.text)
            } catch (e) {
                return []
            }
        }
    ...
    function loadMessages(chatId) {
        var path = root.homeDir + "/.cache/quickshell/chat_sidebar/" + chatId + ".json"
        var fileView = Qt.createQmlObject('import Quickshell.Io; FileView { path: "file://' + path + '"; watchChanges: true }', root)
        if (fileView && fileView.text && fileView.text.trim().length > 0) {
            try {
                return JSON.parse(fileView.text)
    ...
    function saveChatHistory() {
        var path = root.homeDir + "/.cache/quickshell/chat_sidebar/chats.json"
        var fileView = Qt.createQmlObject('import Quickshell.Io; FileView { path: "file://' + path + '" }', root)
        fileView.write(JSON.stringify(chatHistory))
    }

    function saveMessages() {
        if (!currentChatId) return
        var path = root.homeDir + "/.cache/quickshell/chat_sidebar/" + currentChatId + ".json"
        var fileView = Qt.createQmlObject('import Quickshell.Io; FileView { path: "file://' + path + '" }', root)
        fileView.write(JSON.stringify(messages))
    }
    ...
    function deleteChat(index) {
        ...
        saveChatHistory()
        var msgPath = root.homeDir + "/.cache/quickshell/chat_sidebar/" + chatId + ".json"
        var delFile = Qt.createQmlObject('import Quickshell.Io; FileView { path: "file://' + msgPath + '" }', root)
        delFile.remove()
        if (chatHistory.length > 0) {
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    Rectangle {
                        ...
                        color: theme.accent
                        opacity: 0.3
                        Behavior on opacity { ColorAnimation { duration: 150; easing.type: Easing.OutQuad } }
                    }
