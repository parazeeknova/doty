...
fn get_latest_terminal_code_version() -> Option<String> {
    let mut args = vec![
        "-s",
        "-H",
    ...
    args.push("https://api.github.com/repos/zenbu-labs/terminal-code/releases/latest");

    let output = Command::new("curl").args(args).output().ok()?;

    if !output.status.success() {
        eprintln!("Failed to fetch terminal-code release info from GitHub API.");
        return None;
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Checking for terminal-code updates...");
    ...
    // Locate terminal-code block
    let start_idx = content
        .find("pname = \"terminal-code\";")
        .ok_or("Cannot find terminal-code definition in dev.nix")?;
    let tode_block = &content[start_idx..];

    let current_version = extract_field(tode_block, "version = \"", "\";")
        .ok_or("Cannot find current terminal-code version")?;
    let current_hash = extract_field(tode_block, "sha256 = \"", "\";")
        .ok_or("Cannot find current terminal-code hash")?;

    println!("Current local terminal-code version: {}", current_version);

    let latest_version = match get_latest_terminal_code_version() {
        Some(v) => v,
        None => {
            eprintln!("Warning: Could not check for terminal-code updates. Skipping update check.");
            return Ok(());
        }
    };

    println!("Latest online terminal-code version: {}", latest_version);

    if latest_version == current_version {
        println!("terminal-code is already up to date!");
        return Ok(());
    }
    ...
    let new_url = format!(
        "https://github.com/zenbu-labs/terminal-code/releases/download/v{}/tode-linux-x64.tar.gz",
        latest_version
    );
    ...
    let end_idx = content[start_idx..]
        .find("};")
        .ok_or("Cannot find end of terminal-code block")?
        + start_idx
        + 2;
    ...
    println!(
        "Successfully updated dev.nix to terminal-code version {} with hash {}",
    ...
    if should_commit {
        ...
        let commit_msg = format!(
            "chore: auto-update terminal-code to version {}",
            latest_version
        );
        let status = Command::new("git")
    ...
    println!("terminal-code update complete!");
