use std::fs;
use std::path::Path;
use std::process::Command;

fn extract_field(content: &str, start_pattern: &str, end_pattern: &str) -> Option<String> {
    let start_idx = content.find(start_pattern)?;
    let val_start = start_idx + start_pattern.len();
    let end_idx = content[val_start..].find(end_pattern)?;
    Some(content[val_start..val_start + end_idx].to_string())
}

fn get_latest_cap_release() -> Option<(String, String)> {
    let mut args = vec![
        "-s",
        "-H",
        "User-Agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36",
    ];

    let mut token = fs::read_to_string("/run/secrets/github-token")
        .ok()
        .map(|s| s.trim().to_string());

    if token.is_none() {
        token = std::env::var("GITHUB_PERSONAL_ACCESS_TOKEN")
            .ok()
            .or_else(|| std::env::var("GITHUB_TOKEN").ok());
    }

    let auth_header;
    if let Some(t) = token {
        auth_header = format!("Authorization: Bearer {}", t);
        args.push("-H");
        args.push(&auth_header);
    }

    args.push("https://api.github.com/repos/CapSoftware/Cap/releases/latest");

    let output = Command::new("curl").args(&args).output().ok()?;

    if !output.status.success() {
        eprintln!("Failed to fetch Cap release info from GitHub API.");
        return None;
    }

    let json = String::from_utf8_lossy(&output.stdout);
    let tag = extract_field(&json, "\"tag_name\":\"", "\"")?;

    let version = tag
        .strip_prefix("cap-v")
        .or_else(|| tag.strip_prefix('v'))
        .unwrap_or(&tag)
        .to_string();

    let linux_url = if let Some(url) = extract_field(&json, "\\\"linux\\\":\\\"", "\\\"") {
        url
    } else if let Some(url) = extract_field(&json, "\"linux\":\"", "\"") {
        url
    } else if let Some(url) = extract_field(&json, "[Linux](", ")") {
        url
    } else {
        eprintln!("Failed to find Linux download URL in Cap release.");
        return None;
    };

    Some((version, linux_url))
}

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Checking for Cap updates...");

    let desktop_nix_path = Path::new("modules/hosts/apostrophe/packages/desktop.nix");
    if !desktop_nix_path.exists() {
        eprintln!("Error: {:?} not found.", desktop_nix_path);
        std::process::exit(1);
    }

    let content = fs::read_to_string(desktop_nix_path)?;

    // Locate Cap block
    let start_idx = content
        .find("# -- Cap --")
        .ok_or("Cannot find Cap header in desktop.nix")?;
    let cap_block = &content[start_idx..];

    let current_version =
        extract_field(cap_block, "version = \"", "\";").ok_or("Cannot find current Cap version")?;
    let current_url =
        extract_field(cap_block, "url = \"", "\";").ok_or("Cannot find current Cap url")?;
    let current_hash =
        extract_field(cap_block, "sha256 = \"", "\";").ok_or("Cannot find current Cap hash")?;

    println!("Current local Cap version: {}", current_version);

    let (latest_version, new_url) = match get_latest_cap_release() {
        Some(res) => res,
        None => {
            eprintln!("Warning: Could not check for Cap updates. Skipping update check.");
            return Ok(());
        }
    };

    println!("Latest online Cap version: {}", latest_version);

    if latest_version == current_version && new_url == current_url {
        println!("Cap is already up to date!");
        return Ok(());
    }

    println!(
        "New version {} available! Fetching new hash...",
        latest_version
    );

    let output = Command::new("nix-prefetch-url").arg(&new_url).output()?;

    if !output.status.success() {
        eprintln!("Failed to get hash from nix-prefetch-url.");
        std::process::exit(1);
    }

    let new_hash = String::from_utf8_lossy(&output.stdout).trim().to_string();
    if new_hash.is_empty() {
        eprintln!("nix-prefetch-url returned an empty hash.");
        std::process::exit(1);
    }
    println!("Fetched hash: {}", new_hash);

    let content = fs::read_to_string(desktop_nix_path)?;
    let start_idx = content
        .find("# -- Cap --")
        .ok_or("Cannot find Cap header in desktop.nix")?;
    let end_idx = content[start_idx..]
        .find("})")
        .ok_or("Cannot find end of Cap block")?
        + start_idx
        + 2;
    let old_block = &content[start_idx..end_idx];

    let mut new_block = old_block.to_string();
    new_block = new_block.replace(
        &format!("version = \"{}\";", current_version),
        &format!("version = \"{}\";", latest_version),
    );
    new_block = new_block.replace(
        &format!("url = \"{}\";", current_url),
        &format!("url = \"{}\";", new_url),
    );
    new_block = new_block.replace(
        &format!("sha256 = \"{}\";", current_hash),
        &format!("sha256 = \"{}\";", new_hash),
    );

    let mut new_content = content.clone();
    new_content.replace_range(start_idx..end_idx, &new_block);
    fs::write(desktop_nix_path, new_content)?;

    println!(
        "Successfully updated desktop.nix to Cap version {} with hash {}",
        latest_version, new_hash
    );

    let args: Vec<String> = std::env::args().collect();
    let should_commit = args.contains(&"--commit".to_string());
    if should_commit {
        println!("Staging and committing changes to Git...");
        let status = Command::new("git")
            .args(["add", "modules/hosts/apostrophe/packages/desktop.nix"])
            .status()?;
        if !status.success() {
            eprintln!("Failed to run git add.");
            std::process::exit(1);
        }

        let commit_msg = format!("chore: auto-update Cap to version {}", latest_version);
        let status = Command::new("git")
            .args(["commit", "-m", &commit_msg])
            .status()?;
        if !status.success() {
            eprintln!("Failed to run git commit.");
            std::process::exit(1);
        }
        println!("Committed: {}", commit_msg);
    }

    println!("Cap update complete!");

    Ok(())
}
