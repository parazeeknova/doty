...
fn main() {
    ...
    // Fetch calendar html
    if let Some(content) = helpers_rs::run_cmd("curl", &["-sLfg", &calendar_url]) {
        html = content;
    } else {
        fetch_success = false;
    }
    ...
    // Fetch events json
    if fetch_success {
        if let Some(content) = helpers_rs::run_cmd(
            "curl",
...
fn get_local_commit_count(repo_name: &str) -> Option<u32> {
    ...
    for path_str in paths {
        ...
        if path.join(".git").exists() {
            if let Some(output) =
                helpers_rs::run_cmd("git", &["-C", &path_str, "rev-list", "--count", "HEAD"])
            {
